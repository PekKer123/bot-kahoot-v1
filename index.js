        ...
        switch(args[0]) {
            case "help":
                const em = new Discord.RichEmbed()
                    .setColor('#b6da04')
                    .setTitle('คำสั่งสแปมkahoot')
                    .setDescription('***kahoot [รหัสPIN] [จำนวนบอท]**')
                    //.addField('คำสั่งสแปมkahoot', '**+kahoot [รหัสPIN] [จำนวนบอท]**')
                    .addField('Followers Me:', value='[Discord](https://discord.gg/RC4KEDakbd) | [YouTube](https://www.youtube.com/channel/UCUuji8mlYoiKyFP8ROchVsA)', true)
                    .setTimestamp()
                    .setFooter('Spam Kahoot By PekHub', 'https://media.discordapp.net/attachments/928635658199773184/928641191409504306/FB-KFmbWYAQACj0.jpg?width=480&height=480')
...
client.on("message", message => {
    if (message.content.startsWith(Prefix)) {
        ...
        switch(args[0]) {
            ...
            if (args.length == 3) {
                if (args[2] > 2000) {
                    message.channel.send("จำนวนบอทสูงสุดคือ **2000!**").catch(err => {})
                } else {
                ...
                message.channel.send("```\nพิมพ์ให้ถูก => " + Prefix + "kahoot [PIN] [จำนวน]\n```").catch(err => {})
            }
        }
    }
})
